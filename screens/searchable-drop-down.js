...
export default class SearchableDropDown extends Component {
  ...
  renderItems = (item, index) => {
    if(this.props.multi && this.props.selectedItems && this.props.selectedItems.length > 0) {
      return (
          ...
          ? 
          <TouchableOpacity style={{ ...this.props.itemStyle, flex: 1, flexDirection: 'row', justifyContent: "space-around"  }}>
            <View style={{ flex: 0.9, flexDirection: 'row', alignItems: 'center'}}>
              <Image source={{uri: global.image_host_url + item.url}} style={{height: 0.08 * screen.width, width: 0.08 * screen.width, borderRadius: 50}}></Image>
              <Text>  { item.name }</Text>
            </View>
  ...
  renderSelectedItems(){
    ...
    if(items !== undefined && items.length > 0 && this.props.chip && this.props.multi){
     return  <View style={{flexDirection: 'row',  flexWrap: 'wrap', paddingBottom: 0.02 * screen.height, marginTop: 0.01 * screen.height, overflow: "scroll" }}>
                 { items.map((item, index) => {
                     return (
                         <View key={index} style={{
                             ...
                             <Text style={{ color: '#555', alignItems: "center" }}> {item.name}</Text>
                             <TouchableOpacity onPress={() => setTimeout(() => { this.props.onRemoveItem(item, index) }, 0) } style={{ backgroundColor: '#f16d6b', alignItems: 'center', justifyContent: 'center', width: 25, height: 25, borderRadius: 100, marginLeft: 10}}>
                                 <Text>X</Text>
                             </TouchableOpacity>
                         </View>
